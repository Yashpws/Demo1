...
import TestCases.TestCase_Booking_smoke;
...
public class JyperionListener implements ITestListener {
	String browser; String driversyntax; String path;
	/**
	 * Document
	 */
	...
	public void onTestSuccess(ITestResult result) {
		log("onTestSuccess("+result.getName().toString().trim()+")");
		String file=System.getProperty("user.dir")+"\\"+"screenshotpass"+(new Random().nextInt())+".png";
		try{
			TestCase_Booking_smoke.takeSnapShot(file);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public void onTestFailure(ITestResult result) {
		log("onTestFailure("+result.getName().toString().trim()+")");
		String file=System.getProperty("user.dir")+"\\"+"screenshotfail"+(new Random().nextInt())+".png";
		try{
			TestCase_Booking_smoke.takeSnapShot(file);
		}catch(Exception e){
			e.printStackTrace();
		...
		if (throwable != null) {
			...
			Chunk imdb= new Chunk("[SCREEN SHOT]",new Font(Font.TIMES_ROMAN, Font.DEFAULTSIZE, Font.UNDERLINE));
		     ...
		     Paragraph excep1 = new Paragraph(throwable.toString());
		     excep1.add(imdb);
	...
	public void onTestSkipped(ITestResult result) {
		log("onTestSkipped("+result.getName().toString().trim()+")");
		String file=System.getProperty("user.dir")+"\\"+"screenshotskip"+(new Random().nextInt())+".png";
		try{
			TestCase_Booking_smoke.takeSnapShot(file);
		}catch(Exception e){
			e.printStackTrace();
		}
		if (this.skippTable == null) {
			this.skippTable = new PdfPTable(new float[]{.3f, .3f, .1f, .3f});
			this.skippTable.setTotalWidth(20f);
			Paragraph p = new Paragraph("Skipped TESTS", new Font(Font.TIMES_ROMAN, Font.DEFAULTSIZE, Font.BOLD));
			p.setAlignment(Element.ALIGN_CENTER);
			PdfPCell cell = new PdfPCell(p);
			cell.setColspan(4);
			cell.setBackgroundColor(Color.RED);
			this.skippTable.addCell(cell);
		...
		if (throwable != null) {
			...
			this.skippTable.addCell(new PdfPCell(new Paragraph("")));
		}
	}
	...
	public void onStart(ITestContext context) {
		log("onStart("+context.getName().toString().trim()+")");
		try {
	...
	public void onFinish(ITestContext context) {
		log("onFinish("+context.getName().toString().trim()+")");
		...
		try {
			...
			if (this.skippTable != null) {
				...
				this.skippTable.setSpacingBefore(15f);
				this.document.add(this.skippTable);
				this.skippTable.setSpacingBefore(15f);
			}
